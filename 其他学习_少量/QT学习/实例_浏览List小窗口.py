...
class Ui_MainWindow(object):
    ...
    # 获取所有文件
    def getFiles(self):
        #self.lineEdit.text() 为保存的路径
        #self.lineEdit_2.text() 为日期
        self.list = ['effy','allen','1005','11111111111','abcd'] *5 # 列出文件夹下所有的目录与文件
    ...
    # 将文件显示在Table中,把self.list的东西写入表格
    # tableWidget.setItem(行号,列号,内容)
    def bindTable(self):
        for i in range(0, len(self.list)):# 遍历文件列表
            ...
            self.tableWidget.setItem(i, 2, QtWidgets.QTableWidgetItem(self.list[i]))

    # 表格里的内容,单击方法，用来打开选中的项
